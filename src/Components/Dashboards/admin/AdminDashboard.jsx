...
function AdminDashboard() {
    ...
    const [newTaskDueDate, setNewTaskDueDate] = useState(new Date());  // Initialize with current date
    ...
    const [taskReviews, setTaskReviews] = useState({});  // Store reviews: { taskId: { userId: score } }
        ...
        if (newTaskTitle.trim() && newTaskDescription.trim() && newTaskDueDate) {
            try {
                const response = await axiosInstance.post('/task/create', {
                    ...
                    assignedUsers: [], // Initially no one is assigned
                });
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                } else if (selectedMenu === 'View Details') { //Corrected here
    ...
    const handleSubmitReviews = async () => {
        ...
        try {
            // Calculate the total score to be added for the selected campus leader
            let totalNewScore = 0;
            ...
            // Validate that totalNewScore is not NaN
                            ...
                            <Paper sx={{ p: 2, mb: 2 }}>
                                ...
                                <List>
                                    {tasksForSelectedLeader.map(task => {
                                        // Find the assigned user for the selected leader
                                        const assignedUser = task.assignedUsers.find(au => au.user === selectedLeaderId);
                                        const imageUrl = assignedUser ? assignedUser.completionImage : null; // Get completion image URL
...
// import FilterListIcon from '@mui/icons-material/FilterList';
// import MoreVertIcon from '@mui/icons-material/MoreVert';
// // import PeopleIcon from '@mui/icons-icon-button-group/dist/ButtonGroup';
// import PeopleIcon from '@mui/icons-material/People';  // corrected line
// import DescriptionIcon from '@mui/icons-material/Description';
// import TaskIcon from '@mui/icons-material/Task';
// import CloudDownloadIcon from '@mui/icons-material/CloudDownload';
...
//     const [editableRoles, setEditableRoles] = useState(['user', 'campusleader']);
//     const [campusLeaders, setCampusLeaders] = useState([]);
//     const [taskReviews, setTaskReviews] = useState({}); // Store reviews for each task/user combination
//     const campusLeaderOpen = Boolean(campusLeaderAnchorEl);
...
//         if (assignTask && assignedStudent) {
//             try {
//                 // Assuming your API expects taskId and userId for assigning
//                 const response = await axiosInstance.put(`/task/assign-user/${assignTask}`, { userId: assignedStudent }); //  PUT
//                 //  or PATCH
//                 console.log('Assignment Response:', response.data);
//                 alert(`Task assigned successfully!`);
//                 setAssignTask('');
...
//                                     <List>
//                                         {task.assignedUsers && task.assignedUsers.length > 0 ? (
//                                             task.assignedUsers.map(assignedUser => { // Rename 'user' to 'assignedUser' for clarity
//                                                 const userName = assignedUser.user ? assignedUser.user.name : 'Unknown User'; // Safely access user.name
//                                                 return (
//                                                     <ListItem key={assignedUser._id || assignedUser.user._id || assignedUser.user}>
//                                                         <ListItemText primary={userName} />
//                                                     </ListItem>
...
//                                     <FormControl fullWidth sx={{ mb: 2 }}>
//                                         <InputLabel id={`assign-student-label-${task._id}`}>Assign Student</InputLabel>
//                                         <Select
//                                             labelId={`assign-student-label-${task._id}`}
//                                             id={`assign-student-${task._id}`}
//                                             value={assignedStudent}
...
//                                         </Select>
//                                     </FormControl>
//                                     <Button variant="contained" onClick={() => handleAssignTask(task._id, assignedStudent)} sx={{ backgroundColor: '#51437d' }}>
//                                         Assign Task
//                                     </Button>
//                                 </Paper>
//                             ))}
...
//                     <Box sx={{ mt: 2 }}>
//                         {campusLeaders.map(leader => (
//                             <Paper key={leader._id} sx={{ p: 2, mb: 2 }}>
//                                 <Typography variant="h6">{leader.name}</Typography>
//                                 <Typography variant="subtitle1">Assigned Tasks:</Typography>
//                                 <List>
//                                     {tasks.filter(task => task.assignedUsers && task.assignedUsers.some(user => user === leader._id)).map(task => (
//                                         <ListItem key={task._id}>
//                                             <ListItemText primary={task.title} />
//                                             <TextField
//                                                 label="Review/Marks"
//                                                 type="number"
//                                                 value={taskReviews[task._id]?.[leader._id] || ''}
//                                                 onChange={(e) => handleReviewChange(task._id, leader._id, e.target.value)}
//                                                 sx={{ width: '100px', ml: 2 }}
//                                             />
//                                         </ListItem>
//                                     ))}
...
//                 );
//             default:
//                 return <Typography>Hello Admin !</Typography>;
//         }
//     };
...
//                             subMenu: true,
//                         },
//                         { text: 'Review Tasks', icon: <AssignmentIcon />, onClick: () => handleMenuClick('Review Tasks') }, // Added Review Tasks
//                     ].map((item, index) => (
//                         <ListItem
//                             button
//                             key={item.text}
...
//             <Box component="main" sx={{ flexGrow: 1, bgcolor: '#f5f5f5', height: "100vh", p: 3 }}>
//                 <AppBar position="static" color="transparent" elevation={0} sx={{ mb: 3 }}>
//                     <Toolbar sx={{ justifyContent: 'space-between' }}>
//                         <Search sx={{ backgroundColor: '#f0f0f0', borderRadius: 2, width: 300 }}>
//                             <SearchIconWrapper>
...
//                                 <NotificationsIcon />
//                             </IconButton>
//                             <Avatar sx={{ ml: 2, width: 32, height: 32 }} src="/placeholder-avatar.jpg" />
//                         </Box>
//                     </Toolbar>
//                 </AppBar>
...
//     const [allUsers, setAllUsers] = useState([]);  // Store all users fetched from backend
//     const [assignedUsers, setAssignedUsers] = useState([]);
//     const [editableRoles, setEditableRoles] = useState(['user', 'campusleader']); 
...
//         if (assignTask && assignedStudent) {
//             try {
//                 await axiosInstance.post('/task/assignTask', {
//                     taskId: assignTask,
//                     studentId: assignedStudent,
//                 });
//                 console.log(`Task "${assignTask}" assigned to student "${assignedStudent}"`);
//                 alert(`Task "${assignTask}" assigned to student "${assignedStudent}"`);
//                 setAssignTask('');
//                 setAssignedStudent('');
//             } catch (err) {
//                 setError(err.message || 'Error assigning task');
...
//     // const handleDownload = async () => {
//     //     try {
//     //         const response = await axiosInstance.get('/campusLeader/excel-data', {
//     //             responseType: 'blob',
//     //         });

//     //         const blob = new Blob([response.data.applications], { type: 'application/vnd.ms-excel' });
//     //         const url = window.URL.createObjectURL(blob);
...
//                         </FormControl>
//                         <FormControl fullWidth sx={{ mb: 2 }}>
//                             <InputLabel id="assign-student-label">Student</InputLabel>
//                             <Select
...
//                             >
//                                 {assignedUsers.map((student) => (
//                                     <MenuItem key={student._id} value={student.name}>{student.name}</MenuItem>
//                                 ))}
//                             </Select>
//                         </FormControl>
//                         <Button variant="contained" onClick={handleAssignTask} sx={{ backgroundColor: '#51437d' }}>
//                             Assign Task
//                         </Button>
//                     </Box>
//                 );
...
// // Sample data
// const students = [
//   { id: 'SC111001', name: 'Aditi Verma', program: 'MBA', age: 21, gender: 'M', contact: '1234567890', email: 'abc@gmail.com' },
//   { id: 'SC111001', name: 'Student-1', program: 'MCA', age: 19, gender: 'F', contact: '2345678902', email: 'abc@gmail.com' },
//   { id: 'SC111001', name: 'Student-1', program: 'BBA', age: 19, gender: 'F', contact: '3456789012', email: 'uni@gmail.com' },
//   { id: 'SC111001', name: 'Student-1', program: 'B.Com', age: 20, gender: 'M', contact: '4567890123', email: 'uni@gmail.com' },
//   { id: 'SC111001', name: 'Student-1', program: 'M.Com', age: '8.00', gender: 'M', contact: '4567890123', email: 'uni@gmail.com' },
//   { id: 'SC111001', name: 'Student-1', program: '4.00', age: '8.00', gender: 'M', contact: '4567890123', email: 'uni@gmail.com' },
//   { id: 'SC111001', name: 'Student-1', program: '4.00', age: '8.00', gender: 'M', contact: '4567890123', email: 'uni@gmail.com' },
//   { id: 'SC111001', name: 'Student-1', program: '4.00', age: '8.00', gender: 'M', contact: '4567890123', email: 'uni@gmail.com' },
//   { id: 'SC111001', name: 'Student-1', program: '4.00', age: '8.00', gender: 'M', contact: '4567890123', email: 'uni@gmail.com' },
//   { id: 'SC111001', name: 'Student-1', program: '4.00', age: '8.00', gender: 'M', contact: '4567890123', email: 'uni@gmail.com' },
...
// const signupData = [
//   { id: 1, email: 'user1@example.com', role :"user", signupDate: '2024-01-01' },
//   { id: 2, email: 'user2@example.com', role :"user", signupDate: '2024-01-05' },
//   { id: 3, email: 'user3@example.com', role :"user", signupDate: '2024-01-10' },
// ];

// const formData = [
//   { id: 1, campusLeaderName: 'John Doe', college: 'ABC College', formDate: '2024-01-15' },
//   { id: 2, campusLeaderName: 'Jane Smith', college: 'XYZ University', formDate: '2024-01-20' },
// ];
...
//     if (assignTask && assignedStudent) {
//       // Logic to assign task to student (dummy for now)
//       alert(`Task "${assignTask}" assigned to student "${assignedStudent}"`);
//       setAssignTask('');
//       setAssignedStudent('');
//     }
...
//                     <TableCell>Email</TableCell>
//                     <TableCell>Role</TableCell>
//                     <TableCell>Sigup date </TableCell>
//                   </TableRow>
//                 </TableHead>
//                 <TableBody>
//                   {signupData.map((data) => (
...
//                       <TableCell>{data.email}</TableCell>
//                       <TableCell>{data.role}</TableCell>
//                       <TableCell>{data.signupDate}</TableCell>
//                     </TableRow>
//                   ))}
//                 </TableBody>
//               </Table>
...
//                     <TableRow key={data.id}>
//                       <TableCell>{data.id}</TableCell>
//                       <TableCell>{data.campusLeaderName}</TableCell>
//                       <TableCell>{data.college}</TableCell>
//                       <TableCell>{data.formDate}</TableCell>
//                     </TableRow>
//                   ))}
//                 </TableBody>
//               </Table>
...
//         return (
//           <Box sx={{ mt: 2 }}>
//             <Typography>Campus Leader Details:</Typography>
//             <Typography>Name: John Doe</Typography>
//             <Typography>College: ABC College</Typography>
...
//           {[
//             { text: 'Signup Data', icon: <PeopleIcon /> },
//             { text: 'Form Data', icon: <DescriptionIcon /> },
//             {
//               text: 'Campus Leader',
//               icon: <SchoolIcon />,
//               onClick: handleCampusLeaderClick,
...
//         <MenuItem onClick={() => { handleMenuClick('View Student Database'); handleCampusLeaderClose(); }}>View Student Database</MenuItem>
//         <MenuItem onClick={() => { handleMenuClick('Download Details'); handleCampusLeaderClose(); }}>Download Details</MenuItem>
//         <MenuItem onClick={() => { handleMenuClick('View Details'); handleCampusLeaderClose(); }}>View Details</MenuItem>
